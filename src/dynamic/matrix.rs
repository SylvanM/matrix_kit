...
impl<T: Clone> Matrix<T> {
    ...
    pub fn from_index_def(
        ...
        Matrix::from_flatmap(
            rows,
            cols,
            Vec::from_iter((0..(rows * cols)).map(|i| at_index(i % rows, i / rows))),
        )
    }
    ...
    pub fn get_diagonal(&self) -> Vec<T> {
        ...
        for i in 0..min(self.row_count(), self.col_count()) {
            diagonal.push(self.get(i, i));
        }
        diagonal
    }
    ...
    pub fn get_upperdiagonal(&self) -> Vec<T> {
        let mut upper_diagonal: Vec<T> = Vec::new();
        for i in 0..(min(self.row_count(), self.col_count()) - 1) {
            upper_diagonal.push(self.get(i, i + 1));
        }
        upper_diagonal
    }
        ...
        (0..self.col_count())
            ...
            .map(|c| {
                Matrix::from_flatmap(
                    self.row_count(),
                    1,
                    self.flatmap[index!(self.row_count(), self.col_count(), 0, c)
                        ..index!(self.row_count(), self.col_count(), self.row_count(), c)]
                        .to_vec(),
                )
            })
            .collect()
    ...
    pub fn append_mat_right(&mut self, mat: Matrix<T>) {
        ...
        self.col_count += mat.col_count();
        self.flatmap.extend(mat.flatmap);
    }
    ...
    pub fn append_mat_bottom(&mut self, mat: Matrix<T>) {
        ...
        let mut transposed = self.transpose();
        transposed.append_mat_right(mat.transpose());
    ...
    pub fn get_submatrix(&self, row_range: Range<usize>, col_range: Range<usize>) -> Matrix<T> {
        let mut sub_flatmap = Vec::new();
        for c in col_range.clone() {
            for r in row_range.clone() {
                sub_flatmap.push(self.get(r, c));
            }
        }

        Matrix::from_flatmap(row_range.len(), col_range.len(), sub_flatmap)
    }
    ...
    pub fn set_submatrix(
        ...
        for r in row_range.clone() {
            for c in col_range.clone() {
                self.set(r, c, submat.get(r - row_range.start, c - col_range.start));
            }
        }
    }
}
...
impl<R: Ring> Matrix<R> {
    ...
    pub fn zeros(rows: usize, cols: usize) -> Matrix<R> {
    ...
    pub fn from_diagonal(diagonal: Vec<R>) -> Matrix<R> {
        Matrix::from_index_def(diagonal.len(), diagonal.len(), &mut |r, c| {
            if r == c {
                diagonal[r].clone()
            } else {
                R::zero()
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if m == 1 {
            ...
            for c in 0..n {
                ...
                this_line.push(format!(
                    "{}{}",
                    this_entry_str,
                    " ".repeat(widest_str_len - this_entry_len + 1)
                ));
            }
...
impl<R: Ring> Index<usize> for Matrix<R> {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        &self.flatmap[index!(self.row_count, self.col_count, 0, index)
...
impl<R: Ring> IndexMut<usize> for Matrix<R> {
    ...
    fn index_mut(&mut self, index: usize) -> &mut Self::Output {
        &mut self.flatmap[index!(self.row_count, self.col_count, 0, index)
            ..index!(self.row_count, self.col_count, self.row_count, index)]
    }
}
...
impl<R: Ring> Add for Matrix<R> {
    ...
    fn add(self, rhs: Matrix<R>) -> Matrix<R> {
        ...
        let mut out = Matrix::<R>::zeros(self.row_count, self.col_count);
        mat_add(
            self.row_count,
            self.col_count,
            &self.flatmap,
...
impl<R: Ring> MulAssign<R> for Matrix<R> {
    fn mul_assign(&mut self, rhs: R) {
        scalar_mul_assign(self.row_count * self.col_count, rhs, &mut self.flatmap);
    }
}
...
impl<F: Field> Div<F> for Matrix<F> {
    ...
    fn div(self, rhs: F) -> Matrix<F> {
        let mut out = Matrix::<F>::zeros(self.row_count, self.col_count);
        scalar_div(
            self.row_count * self.col_count,
            rhs,
            &self.flatmap,
...
impl<F: Field> DivAssign<F> for Matrix<F> {
    fn div_assign(&mut self, rhs: F) {
        scalar_div_assign(self.row_count * self.col_count, rhs, &mut self.flatmap);
    }
}
...
impl<R: Ring> Mul<Matrix<R>> for Matrix<R> {
    ...
    fn mul(self, rhs: Matrix<R>) -> Self::Output {
        assert_eq!(
            self.col_count,
            rhs.row_count,
            "Attempting to multiply a {} x {} matrix with a {} x {} matrix.",
            self.row_count(),
            self.col_count(),
            rhs.row_count(),
            rhs.col_count()
        ...
        let mut out = Matrix::<R>::zeros(self.row_count, rhs.col_count);
        mat_mul_ptrs::<R>(
            self.row_count,
            self.col_count,
            rhs.col_count,
...
impl<R: Ring> MulAssign<Matrix<R>> for Matrix<R> {
    ...
    fn mul_assign(&mut self, rhs: Matrix<R>) {
        ...
        mat_mul_ptrs_assign(self.row_count, &mut self.flatmap, &rhs.flatmap);
    }
}
...
mod matrix_tests {
    ...
    #[test]
    fn test_index_def() {
        let a = Matrix::from_index_def(10, 9, &mut { |r, c| if r == c { 1 } else { 0 } });
